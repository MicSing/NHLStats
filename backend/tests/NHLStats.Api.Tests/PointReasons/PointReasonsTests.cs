...
public class PointReasonsTests : ApiTestBase
{
    ...
    {
        var client = Factory.CreateClient();
        var resp = await client.PostAsJsonAsync("/api/pointreasons", new { name = "Test", pointType = "Negative" });
    ...
    public async Task Create_returns_201_with_reason()
    {
        ...
        body.GetProperty("pointType").GetString().Should().Be("Positive");
    ...
    public async Task Update_returns_200_with_updated_reason()
    {
        ...
        updated.GetProperty("pointType").GetString().Should().Be("Positive");
    }
