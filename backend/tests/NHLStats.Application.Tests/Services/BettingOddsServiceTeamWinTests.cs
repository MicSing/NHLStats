...
public class BettingOddsServiceTeamWinTests : IDisposable
{
    ...
    public async Task ReproducesPythonReference_25Games_L10HotStreak_2H2H()
    {
        ...
        var margin = OddsFormula.MatchMargin(upcoming.Id);
        hostedRow.Odds.Should().Be(Math.Floor((1m + (1m / expectedHosted - 1m) * margin) * 100m) / 100m);
        oppRow.Odds.Should().Be(Math.Floor((1m + (1m / expectedOpp - 1m) * margin) * 100m) / 100m);
